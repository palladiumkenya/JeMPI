#!/bin/bash

TOPIC_PATIENT_ASYNC_ETL="JeMPI-async-etl"
TOPIC_PATIENT_CONTROLLER="JeMPI-patient-controller"
TOPIC_PATIENT_EM="JeMPI-patient-em"
TOPIC_PATIENT_LINKER="JeMPI-patient-linker"
TOPIC_MU_LINKER="JeMPI-mu-linker"
TOPIC_NOTIFICATIONS="JeMPI-notifications"
TOPIC_BACK_PATCH_DWH="JeMPI-back-patch-dwh"

declare -a TOPICS=(
  $TOPIC_PATIENT_ASYNC_ETL
  $TOPIC_PATIENT_CONTROLLER
  $TOPIC_PATIENT_EM
  $TOPIC_PATIENT_LINKER
  $TOPIC_MU_LINKER
  $TOPIC_NOTIFICATIONS
  $TOPIC_BACK_PATCH_DWH
)

declare -A PARTITIONS
PARTITIONS[$TOPIC_PATIENT_ASYNC_ETL]=1
PARTITIONS[$TOPIC_PATIENT_CONTROLLER]=1
PARTITIONS[$TOPIC_PATIENT_EM]=1
PARTITIONS[$TOPIC_PATIENT_LINKER]=3
PARTITIONS[$TOPIC_MU_LINKER]=1
PARTITIONS[$TOPIC_NOTIFICATIONS]=1
PARTITIONS[$TOPIC_BACK_PATCH_DWH]=1
  
declare -A REPLICATION
REPLICATION[$TOPIC_PATIENT_ASYNC_ETL]=2
REPLICATION[$TOPIC_PATIENT_CONTROLLER]=2
REPLICATION[$TOPIC_PATIENT_EM]=2
REPLICATION[$TOPIC_PATIENT_LINKER]=2
REPLICATION[$TOPIC_MU_LINKER]=2
REPLICATION[$TOPIC_NOTIFICATIONS]=2
REPLICATION[$TOPIC_BACK_PATCH_DWH]=2

declare -A RETENTION_MS
RETENTION_MS[$TOPIC_PATIENT_ASYNC_ETL]=`echo "60*24*60*60*1000" | bc`
RETENTION_MS[$TOPIC_PATIENT_CONTROLLER]=`echo "60*24*60*60*1000" | bc`
RETENTION_MS[$TOPIC_PATIENT_EM]=`echo "60*24*60*60*1000" | bc`
RETENTION_MS[$TOPIC_PATIENT_LINKER]=`echo "60*24*60*60*1000" | bc`
RETENTION_MS[$TOPIC_MU_LINKER]=`echo "60*24*60*60*1000" | bc`
RETENTION_MS[$TOPIC_NOTIFICATIONS]=`echo "60*24*60*60*1000" | bc`
RETENTION_MS[$TOPIC_BACK_PATCH_DWH]=`echo "60*24*60*60*1000" | bc`

declare -A SEGMENT_BYTES
SEGMENT_BYTES[$TOPIC_PATIENT_ASYNC_ETL]=`echo "4*1024*1024" | bc`
SEGMENT_BYTES[$TOPIC_PATIENT_CONTROLLER]=`echo "4*1024*1024" | bc`
SEGMENT_BYTES[$TOPIC_PATIENT_EM]=`echo "4*1024*1024" | bc`
SEGMENT_BYTES[$TOPIC_PATIENT_LINKER]=`echo "4*1024*1024" | bc`
SEGMENT_BYTES[$TOPIC_MU_LINKER]=`echo "4*1024*1024" | bc`
SEGMENT_BYTES[$TOPIC_NOTIFICATIONS]=`echo "4*1024*1024" | bc`
SEGMENT_BYTES[$TOPIC_BACK_PATCH_DWH]=`echo "4*1024*1024" | bc`
